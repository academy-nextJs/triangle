...
import { getAllhouses } from "@/utils/services/api/Landing/SeelandBay/SeelandBoy";
        ...
        {houses.map((house, index) => (
          <div key={house.id || index}>
            <Card className="bg-white shadow-lg rounded-[40px] w-[391px] h-[448px] overflow-hidden">
               <div className="flex justify-center items-center w-full h-[221px] relative">
                {house.photos?.length > 5 ? (
                  <Image
                    src={house.photos[0]}
                    alt={`تصویر ${house.title}`}
                    fill
              ...
              <div className="flex flex-col justify-between h-[calc(438px-221px)] mr-4 text-right">
                ...
                <div className="flex items-end justify-end">
                  <span className="text-black text-[20px] font-bold">
                    {house.price
                      ? new Intl.NumberFormat("fa-IR").format(house.price) +
                        " تومان"
                      : "۱,۵۰۰,۰۰۰ تومان"}
                  </span>
                </div>
